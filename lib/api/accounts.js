...
module.exports = function (app) {
	...
	function Account() {
		this.validAddress = address =>
			(typeof address === 'string' && address.match(/^[0-9]{1,21}[R]$/g) || address.match(/^rise1[a-z0-9]+$/g));
		...
		this.getDelegate = (account, cb) => {
			...
			return request.get({
				...
				if (err || (response.statusCode !== 200 && response.statusCode !== 404)) {
					return cb(err || 'Response was unsuccaessful');
				} else if (body.success === true) {
					account.delegate = body.delegate;
					account.delegate.rate = body.info.rankV2;
					account.delegate.rate = body.info.rankV2;
					account.delegate.approval = body.info.approval;
					account.delegate.productivity = body.info.productivity;
				} else {
		...
		this.getVotes = (account, cb) => {
			...
			return request.get({
				url: `${app.get('lisk address')}/api/accounts/votes/?address=${coreUtils.parseAddress(account.address)}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
				} else if (body.success === true) {
					account.votes = (body.votes !== undefined &&
						body.votes !== null && body.votes.length > 0) ?
						body.votes : null;
					async.series(account.votes.map((delegateName) => (cb) => {
						request.get({
							url: `${app.get('lisk address')}/api/delegates/get?username=${delegateName}`,
							json: true,
						}, (err, response, body) => {
							if (err) {
								return cb(err);
							}
							return cb(null, body.delegate);
						});
					}), (err, results) => {
						if (err) {
							return cb(err);
						}
						account.votes = results;
						cb(null, account);
					});
					return;
				} else {
		...
		this.getVoters = (account, cb) => {
			...
			return request.get({
				url: `${app.get('lisk address')}/api/delegates/voters?username=${account.username}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuaccessful');
				} else if (body.success === true) {
					account.voters = (body.voters !== undefined &&
						body.voters !== null && body.voters.length > 0) ?
						body.voters : null;
		...
		this.getForged = (account, cb) => {
			...
			return request.get({
				url: `${app.get('lisk address')}/api/delegates/rewards?username=${account.username}&from=${Date.parse('2016-05-24T17:00:00.000Z') /1000}&to=${Date.now()/1000}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
				} else if (body.success === true) {
					account.delegate.forged = body.cumulative;
				} else {
