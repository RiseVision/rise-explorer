...
module.exports = function (app) {
	...
	function Active() {
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('lisk address')}/api/delegates/?orderBy=rank:asc&limit=199`,
				json: true,
			}, (err, response, body) => {
	...
	this.getStandby = function (n, error, success) {
		...
		request.get({
			url: `${app.get('lisk address')}/api/delegates/?orderBy=rank:asc&limit=${delegates.limit}&offset=${delegates.actualOffset}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
