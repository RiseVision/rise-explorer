...
module.exports = function (app) {
	...
	function Active() {
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('lisk address')}/api/delegates/?orderBy=votesWeight:desc&limit=199`,
				json: true,
			}, (err, response, body) => {
		...
		this.getForged = function (delegate, cb) {
			cachedRequest({
				url: `${app.get('lisk address')}/api/delegates/rewards?username=${delegate.username}&from=${Date.parse('2016-05-24T17:00:00.000Z') /1000}&to=${Date.now()/1000}`,
				json: true,
			}, app.locals.redis, 180, (err, body) => {
				if (err) {
					return cb(err || 'Response was unsuccessful');
				} else if (body.success === true) {
	...
	function Standby(n) {
		...
		this.actualOffset = (isNaN(this.offset)) ? 199 : this.offset + 199;
	...
	this.getStandby = function (n, error, success) {
		...
		request.get({
			url: `${app.get('lisk address')}/api/delegates/?orderBy=votesWeight:desc&limit=${delegates.limit}&offset=${delegates.actualOffset}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
	...
	this.getSearch = function (params, error, success) {
		if (!params || !params.match(/^(?![0-9]{1,21}[R]$)[0-9a-z.]+/i)) {
