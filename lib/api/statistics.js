...
module.exports = function (app) {
	...
	function Blocks() {
		this.maxCount = Math.floor(86400/app.get('blockTime'));
		this.maxOffset = Math.floor(this.maxCount/100)*100;
	...
	this.getBlocks = function (query, error, success) {
		...
		this.getLastBlock({}, error, ({block}) => {
			async.parallelLimit(
				new Array(blocks.maxCount - 1).fill(0)
				.map((a, idx) => block.height - idx - 1)
				.map((height) => (cb) => cachedRequest({
				...
				(err, resBlocks) => {
					...
					blocks.inspect(resBlocks.concat(block), offset);
					return success({
						success: true,
	...
	this.getLastBlock = function (query, error, success) {
		...
		cachedRequest({
			...
			json: true,
		}, app.locals.redis, app.get('blockTime'), (err, body) => {
			if (err) {
