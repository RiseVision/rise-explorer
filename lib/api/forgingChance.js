...
module.exports = function (app) {
	...
	function Calculator(round) {
		cache[round] = cache[round] || {};
		this.delegates = function (cb) {
			if (cache[round].delegates) {
				return cb(null, cache[round].delegates);
			}
			async.parallel([
					ccb => {
						request.get({
