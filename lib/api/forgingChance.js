		...
		this.delegates = function (cb) {
			...
			async.parallel(
				[
					(ccb) => {
						request.get({
							url : `${app.get('lisk address')}/api/delegates/?orderBy=votesWeight:desc&limit=101`,
							json: true,
						}, (err, response, body) => ccb(err, body.delegates));
					},
					(ccb) => {
						request.get({
							url : `${app.get('lisk address')}/api/delegates/?orderBy=votesWeight:desc&limit=101&offset=101`,
							json: true,
						}, (err, response, body) => ccb(err, body.delegates));
					},
				],
